...
if discriminant < 0 
  puts "#{discriminant} Корней нет"
    ...
    puts "#{discriminant} он имеет один корень #{-b/(a*2)}"   # D = -b/2a
  else discriminant > 0
    puts "#{discriminant} он имеет два корня #{( -b - Math.sqrt(discriminant))/(a * 2)} и #{( -b + Math.sqrt(discriminant))/(a * 2)}"   #Math.sqrt
end      
